<template>
    ...
    <adminLayout>
        ...
        <div v-if="categories.data.length > 0" class="flex justify-center my-4">
            <PaginationList :Links="categories.links" />
        </div>
    </adminLayout>
</template>
...
const categoryCount = computed(() => props.categories.data.length);

const editCategory = (category) => {
    router.visit(route("admin.categories.edit", category));
};

const deleteCategory = (category) => {
    Swal.fire({
        title: "Are you sure? All products will be deleted!",
